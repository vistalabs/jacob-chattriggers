        ...
        if (foundToday) {
            ...
            for (let i = 0; i < lore.length; ++i) {
                ...
                if (match) {
                    let time;
					//ChatLib.chat(jacobs);
					//ChatLib.chat(jacobs.length);
					//ChatLib.chat(jacobs[0]);
...
register('guiKey', (char, keyCode, gui, event) => {
    ...
    if (keyCode === 35) {
        ...
        if (inv.getName().match(/(?:Early )?(?:Summer|Winter|Spring|Autumn), Year \d+/)) {
            scanCalendar(inv);
            ChatLib.chat('&5[JU]&a Done');
        }
    }
});

register("command", () => {
    ChatLib.chat("&5[JU]&b Started upload...");
    var dictstring = jacobs.map(i => {
            return {
    ...
    try {
        request({
            url: 'http://localhost:5000/upload', // change this too
            method: 'POST',
            body: {
                secret: 'ASDF',
                events: dictstring
        ...
        ChatLib.chat(`&5[JU]&4 ERROR: &c${error}`)
    }
